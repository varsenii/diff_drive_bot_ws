...
from diff_drive_bot_interfaces.msg import Slot, Intent
...
class Listen(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("access_key", "Pn8m2f8Msz7LNgAhoSG3GIryNqMeAJbg5dcIGQrbMSmuKTpLSj00kA==", ParameterDescriptor(description="Picovoice API key"))
        ...
        if (self.use_jetson_nano == False):
            self.declare_parameter("keyword_path", ament_index_python.get_package_share_directory(
                "listen_talk_ros2")+ "/Hey-robot_en_linux_v3_0_0.ppn")
            self.declare_parameter("context_path", ament_index_python.get_package_share_directory(
                "listen_talk_ros2")+ "/Mobile-robot_en_linux_v3_0_0.rhn")
    ...
    def _inference_callback(self,inference):
        if inference.is_understood:
            intent = Intent()
            intent.name = inference.intent 
            intent.slots = []

            print('{')
            print("  intent : '%s'" % inference.intent)
            print('  slots : {')
            for name, value in inference.slots.items():
