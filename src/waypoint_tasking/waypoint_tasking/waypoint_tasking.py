...
class WaypointTasker(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "map", "", ParameterDescriptor(description="The map to load waypoints for")
        )
        ...
        self.start_navigation_service = self.create_service(
            StartWaypointTasks,
            "start_waypoint_tasks",
            self.start_waypoint_tasks_callback,
            qos_profile=qos_profile,
        )
        self.sub_intent = self.create_subscription(
            Intent, "/intent", self.intent_callback, 10
        )
        ...
        self.supported_tasks = {"face_recognition": self.face_recognition_manager}
    ...
    def start_waypoint_tasks_callback(self, request, response):
        intialized = self.init_navigation_and_task_execution(
            request.waypoints, request.tasks, request.return_to_start
        )
    ...
    def init_navigation_and_task_execution(
        self, waypoint_indexes, tasks, return_to_start
    ):
        self.logger.info(f"Tasks to perform: {tasks}")
        self.logger.info(
            f"Waypoints to navigate: {'all' if len(waypoint_indexes) == 0 else waypoint_indexes}"
        )
    ...
    def move_to_waypoint(self, waypoint):
        ...
        if self.is_coming_back_to_starting_point:
            self.is_coming_back_to_starting_point = False
            self.logger.info(
                "The robot has successfully returned to the starting point"
            )
    ...
    def on_nav_goal_succeed(self):
        ...
        self.logger.info(
            f"The robot has arrived to the waypoint {self.current_waypoint_index}"
        )
    ...
    def on_task_complete(self, task, result):
        ...
        self.logger.info(
            f"Executed tasks: {self.executed_tasks_per_waypoint}/{len(self.tasks)} at waypoint {self.current_waypoint_index}"
        )

        self.report_manager.log_task_result(
            waypoint_index=self.current_waypoint_index, task=task, result=result
        )
    ...
    def proceed_to_next_waypoint(self):
        ...
        if len(self.tasks) != 0:
            if self.executed_tasks_per_waypoint < len(self.tasks):
                ...
                self.logger.info(
                    f"All tasks are completed at waypoint {self.current_waypoint_index}"
                )
        ...
        if self.current_waypoint_index < len(self.waypoints_to_visit):
            ...
            self.navigation_manager.move_to_waypoint(
                self.waypoint_manager.starting_pose_goal
            )
