...
class NavigationManager:
    ...
    def _cancel_callback(self, future):
        try:
            ...
            if cancel_result and cancel_result.return_code == 0:
                self.logger.info("Goal successfully canceled.")
            else:
                self.logger.warn(
                    "Failed to cancel the goal or it was already completed."
                )
        except Exception as e:
            self.logger.error(f"Error during goal canceling: {e}")
