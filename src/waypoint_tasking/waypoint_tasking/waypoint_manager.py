...
from waypoint_tasking.utils import serialize_markers, deserialize_markers
...
class WaypointManager:
    def __init__(self, node):
        ...
        self.save_waypoints_service = self.node.create_service(
            SaveWaypoints, "/save_waypoints", self.save_waypoints_callback
        )

        self.logger = self.node.get_logger()
        self.waypoints = []
        self.waypoint_dir = os.path.join(
            os.path.curdir, "src", "waypoint_tasking", "waypoint_tasking", "waypoints"
        )
